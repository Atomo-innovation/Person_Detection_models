...
class RTSPDurationReader:
    ...
    open() is defensive about flaky RTSP servers:
      - Sets explicit open/read timeouts (OpenCV >=4.5) so a dead/slow camera
        fails fast instead of hanging on the default (very long) timeout.
      - Retries a few times before giving up.
      - Falls back from TCP to UDP transport if TCP repeatedly fails to open
        (some cameras/NVRs only accept UDP, or block TCP under load).
    """

    def __init__(self, url: str, transport: str = "tcp",
                 connect_timeout_s: float = 8.0, connect_retries: int = 3):
        self.url = url
        self.transport = transport
        self.connect_timeout_s = connect_timeout_s
        self.connect_retries = connect_retries
        self._cap: cv.VideoCapture | None = None
        self.opened_transport: str = transport  # records which transport actually worked

    def _try_open(self, proto: str, timeout_s: float) -> cv.VideoCapture | None:
        os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = (
            "rtsp_transport;{}|fflags;nobuffer|flags;low_delay|max_delay;0|"
            "stimeout;{}".format(proto, int(timeout_s * 1_000_000))  # stimeout is in microseconds
        )
        cap = cv.VideoCapture(self.url, cv.CAP_FFMPEG)
        # OpenCV >=4.5.3: explicit timeouts so a dead camera fails fast
        # instead of hanging on FFmpeg's much longer internal default.
        try:
            cap.set(cv.CAP_PROP_OPEN_TIMEOUT_MSEC, timeout_s * 1000)
            cap.set(cv.CAP_PROP_READ_TIMEOUT_MSEC, timeout_s * 1000)
        except Exception:
            pass  # older OpenCV: silently ignored, env var stimeout still applies
        ...
        # isOpened() can be True even if the stream never actually delivers
        # frames (some RTSP servers accept the TCP handshake but stall).
        # Confirm we can read at least one real frame before trusting it.
        ret, _ = cap.read()
        if not ret:
            cap.release()
            return None
    ...
    def open(self) -> bool:
        attempts: list[str] = []
        primary = "tcp" if self.transport == "tcp" else "udp"
        fallback = "udp" if primary == "tcp" else "tcp"

        for attempt in range(1, self.connect_retries + 1):
            log.info("RTSP connect attempt %d/%d (%s, timeout=%.0fs): %s",
                      attempt, self.connect_retries, primary, self.connect_timeout_s, self.url)
            cap = self._try_open(primary, self.connect_timeout_s)
            if cap is not None:
                self._cap = cap
                self.opened_transport = primary
                return True
            attempts.append("{} attempt {} failed".format(primary, attempt))
            time.sleep(1.0)

        # Primary transport exhausted — try the fallback transport once.
        log.warning("RTSP %s transport failed after %d attempts, trying %s as fallback...",
                    primary, self.connect_retries, fallback)
        cap = self._try_open(fallback, self.connect_timeout_s)
        if cap is not None:
            self._cap = cap
            self.opened_transport = fallback
            log.info("RTSP connected using fallback transport: %s", fallback)
            return True

        log.error("RTSP connection failed entirely. Things to check:\n"
                  "  - Camera URL/credentials correct? (%s)\n"
                  "  - Is the camera reachable? Try: ffprobe \"%s\"\n"
                  "  - Try the other transport explicitly: --rtsp-transport %s\n"
                  "  - Is another process already holding the stream open (max client limit)?",
                  self.url, self.url, fallback)
        return False
    ...
    *,
    ...
    if is_rtsp:
        reader = RTSPDurationReader(rtsp_url, rtsp_transport,
                                     connect_timeout_s=rtsp_connect_timeout_s,
                                     connect_retries=rtsp_connect_retries)
        if not reader.open():
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--rtsp-connect-timeout", type=float, default=8.0,
                   help="Seconds to wait for the RTSP connection/first frame before retrying")
    p.add_argument("--rtsp-connect-retries", type=int, default=3,
                   help="Connection attempts on the primary transport before falling back")
...
def main() -> None:
    ...
    # ── RTSP preflight: fail fast with a clear error before loading any model
    # weights, instead of discovering a dead camera after model A's NPU init. ──
    if is_rtsp:
        log.info("Preflight: checking RTSP connectivity to %s ...", args.rtsp)
        probe = RTSPDurationReader(args.rtsp, args.rtsp_transport,
                                    connect_timeout_s=args.rtsp_connect_timeout,
                                    connect_retries=args.rtsp_connect_retries)
        if not probe.open():
            sys.exit(
                "Cannot reach RTSP stream: {}\n"
                "Verify the URL/credentials and that the camera is reachable, e.g.:\n"
                "  ffprobe \"{}\"\n"
                "or try: --rtsp-transport {}".format(
                    args.rtsp, args.rtsp,
                    "udp" if args.rtsp_transport == "tcp" else "tcp",
                )
            )
        log.info("Preflight OK — stream reachable via %s, frame size %s.",
                  probe.opened_transport, probe.frame_size)
        probe.release()
    ...
    for lib, mdl, name in model_specs:
        m = run_one_model(
            ...
            rtsp_connect_timeout_s=args.rtsp_connect_timeout,
            rtsp_connect_retries=args.rtsp_connect_retries,
            outdir=args.outdir,
